...
/// Documentation-only constant recording the historical Ollama provider
/// temperature default (`0.3`) used before per-call temperatures were
/// universally plumbed through. The `OllamaTuning` struct no longer
/// applies this value automatically — when `temperature_override` is
/// `None` the per-call temperature wins. Retained as a reference point
/// for operators tuning `ollama_temperature_override` in `config.toml`.
pub const OLLAMA_DEFAULT_TEMPERATURE: f64 = 0.3;
